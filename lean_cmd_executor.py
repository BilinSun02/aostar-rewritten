...
class Lean3Executor(object):
    ...
    def compute_complexity(self) -> int:
        '''
        Given the current state of the executor, compute the sum of exp over the lenth of all the goals
        '''
        goal_sum = 0
        for obligation in self.proof_context.fg_goals + self.proof_context.bg_goals:
            goal_sum += sum([ len(hypothesis) for hypothesis in obligation.hypotheses]) + len(obligation.goal)
        return goal_sum
