...
class RPCServer(ABC):
    ...
    def run(self) -> None:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            sock.listen()
            print(f"{type(self).__name__} instance started and listening to " +\
                  f"{self.print_friendly_host}:{self.port}")
            while True:
    ...
    def __handle__(self, client:socket.socket) -> None:
        while True:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to send response: {e.__repr__()}")
